...
def run_ingestion_pipeline(handle):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for index, tweet_container in enumerate(tweet_window):
                ...
                # 1. Extract Unique Identifiers (URL & ID)
                ...
                # 2. SEMANTIC FIX: Extract the true ISO timestamp from the HTML attribute
                time_element = tweet_container.query_selector('time')
                if time_element:
                    payload["timestamp"] = time_element.get_attribute('datetime')

                # 3. SEMANTIC FIX: Extract the exact tweet text using the dedicated data test ID attribute
                text_element = tweet_container.query_selector('[data-testid="tweetText"]')
                if text_element:
                    payload["text"] = text_element.inner_text().strip()
                else:
                    # Fallback for media-only posts or empty quote tweets
                    payload["text"] = "[Media Only or No Content text parsed]"

                # Guardrail validation: Make sure we successfully fetched both keys
    ...
    if not extracted_payloads:
        log_pipeline_health(handle, "EMPTY", "No semantic payloads resolved from active browser view window.")
        return
